...
from ErzaScarlet import dispatcher, VILLIANS, DEV_USERS
...
def connect_chat(update, context):
    ...
    if update.effective_chat.type == "private":
        if args and len(args) >= 1:
            ...
            if (isadmin) or (isallow and ismember) or (user.id in VILLIANS):
...
def connected(bot: Bot, update: Update, chat, user_id, need_admin=True):
    ...
    if chat.type == chat.PRIVATE and sql.get_connected_chat(user_id):
        ...
        if ((isadmin) or (isallow and ismember) or (user.id in VILLIANS) or
            (user.id in DEV_USERS)):
            if need_admin is True:
                if (getstatusadmin.status in ("administrator", "creator") or
                        user_id in VILLIANS or user.id in DEV_USERS):
                    return conn_id
                else:
                    send_message(
                        update.effective_message,
